  ...
  const handleExportExcel = () => {
    ...
    const excelData = analytics.map((batchAnalytics: any) => {
      ...
      const row: any = {
        ...
        'Date': scheduleDate,
        'Submitted At': batchAnalytics.createdAt ? (() => {
          const date = new Date(batchAnalytics.createdAt);
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow className="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-600 hover:to-red-700">
                              ...
                              <TableHead className="text-white font-bold py-4 px-4 text-center">Subject</TableHead>
                              <TableHead className="text-white font-bold py-4 px-4 text-center">Phase</TableHead>
                              <TableHead className="text-white font-bold py-4 px-4 text-center">Schedule Date</TableHead>
                              <TableHead className="text-white font-bold py-4 px-4 text-center">Submitted At</TableHead>
                          ...
                          <TableBody>
                            {analytics.map((batchAnalytics: any, index: number) => (
                              <TableRow key={`${batchAnalytics.staffId || ''}-${batchAnalytics.batchId}`} className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-blue-50 transition-colors`}>
                                ...
                                <TableCell className="text-center py-4 px-4 font-semibold text-blue-700">{batchAnalytics.subjectName || 'N/A'}</TableCell>
                                ...
                                <TableCell className="text-center py-4 px-4 text-sm">{batchAnalytics.createdAt ? (() => {
                                  const date = new Date(batchAnalytics.createdAt);
                                  return `${date.getDate()}-${date.toLocaleString('en-US', { month: 'short' })}-${date.getFullYear()} ${date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`;
                                })() : 'N/A'}</TableCell>
